...
Brand, home page copy, built-in skill enablement, and model IDs live in `app.config.ts`. Edit that file for name, tagline, composer placeholders, which default skills are on or suggested, chat/image/embedding/transcription models, related-session memory thresholds, reminder-email context floors, and whether the signed-in setup page is shown (`setup.inAppPage`). Do not re-hardcode those values in `lib/brand.ts`, `lib/suggested-skills.ts`, or feature modules — they already read the config. Secrets, host URLs, and access allowlists (`ALLOWED_SIGNUP_EMAILS`, `ALLOWED_SIGNUP_DOMAINS`, `BLOCKED_ACCESS_EMAILS`, `ALLOWED_ACCOUNT_USAGE_EMAILS`) stay in `.env.local` / Vercel env. Optional `VERCEL_TOKEN` reads the Vercel billing plan so Pro unlocks sub-daily schedules; without it, assume Hobby. Never put emails or domains in `app.config.ts`.

...
description: >
  ...
  when the user writes /text-to-image.
---
...
If the user invoked this skill with `/text-to-image` and no extra brief, ask subject, setting, mood or style, and where they will use the image. Then write a generation prompt and call `generate_image`.
